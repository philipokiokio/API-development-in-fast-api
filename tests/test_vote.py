...
def test_vote_on_post(authorized_client,test_user,test_post):
...
def test_delete_vote_unexist(authorized_client, test_post):
    res = authorized_client.post('/vote/', json={'post_id': test_post[3].id, 'dir':0})

    assert res.status_code ==404


def test_delete_vote_post_unexist(authorized_client):
    res = authorized_client.post('/vote/', json={'post_id':2, 'dir':0})

    assert res.status_code ==404


def test_vote_user_unexist(client, test_post):
    res = client.post('/vote/', json={'post_id':test_post[0].id, 'dir':0})
